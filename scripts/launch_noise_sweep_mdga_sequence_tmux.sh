...
if (( $# > 0 )); then
  ...
  SESSION_NAME="rmo_mdga_$(date -u +%Y%m%dT%H%M%SZ)"
fi
SEQUENCE_ARGS=("$@")
OUTPUT_DIR="$ROOT_DIR/outputs/noise_sweep_mdga"
...
TMUX_CMD="cd '$ROOT_DIR' && \
  bash scripts/run_noise_sweep_mdga_sequence.sh$SEQUENCE_ARGS_Q 2>&1 | tee '$LOG_FILE'; \
  rc=\${PIPESTATUS[0]}; \
  printf '%s\n' \"\$rc\" > '$STATUS_FILE'; \
  echo; echo exit=\$rc; \
  exec bash"
